...
class Water(unittest.TestCase):
    ...
    def test_cost(self):
        ...
        loss_ref = {1: 58.51884707051761, 2: 413.07612355725223}

        mlo = lo.fourth_moment.FourthMoment(mol, mo_coeff)
        for exponent in [1, 2]:
    ...
    def test_grad_hess(self):
        ...
        for exponent in [1, 2]:
            ...
            self.assertAlmostEqual(abs(g - num_g).max(), 0, delta=1e-3)
            self.assertAlmostEqual(abs(H - num_H).max(), 0, delta=1e-1)
...
def _num_hess(func, x0, step_length):
    ...
    for i in range(n):
        ...
        for j in range(i + 1, n):
            ...
            ybf = func(x0 - dxi + dxj)
            ybb = func(x0 - dxi - dxj)
            H[i, j] = H[j, i] = (yff + ybb - yfb - ybf) / (4.0 * step_length**2.0)
        yf = func(x0 + dxi)
        yb = func(x0 - dxi)
        H[i, i] = (yf + yb - 2 * y0) / step_length**2.0

    return H


if __name__ == "__main__":
    print("Full Tests for FourthMoment")
    unittest.main()
